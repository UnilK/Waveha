...
void Pitcher1::set_absolute_pitch_shift(float s){
    ...
    int nmax = std::ceil(bsize / shift);
    if(nmax + 2 > obuff.size()) obuff.resize(nmax + 2, 0.0f);
...
int Pitcher1::get_delay(){ return bsize + enveloper.get_delay(); }

void Pitcher1::calc_scale(){
    ...
    delayer(64, 0.0f)
{
    ...
    bsize = std::max<int>(max + pop, 2 * max / 3 + maxlen / 2 + 2);
        ...
        while(w.ipointer <= w.fpointer){
            ...
            if(w.state <= 0){
...

                {
                    ...
                    while(j < w.length / 2){
                        obuff[i] += amplitude * std::cos(phase + j * frot)
                            * (1.0f - std::cos((j + w.length / 2) * wrot));
                        i++;
                        j += w.shift;
                    }
                }
                ...
                {
                    int i = -1;
                    float j = w.fpointer - w.ipointer - w.shift;
...
int Pitcher1::get_delay(){ return bsize + enveloper.get_delay() + obuff.left(); }
