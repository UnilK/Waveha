...
std::vector<float> Resampler::process(float sample){
...
void Resampler::set_shift(float pitch_shift){
    shift = std::max(0.05f, std::min(pitch_shift, 20.0f));
    osize = std::max<int>(osize, std::ceil(2 * wsize / shift));
    if(8 * osize > (int)obuff.size()) obuff.resize(8 * osize, 0.0f);
}

int Resampler::get_delay(){ return 2 * wsize - 1; }
